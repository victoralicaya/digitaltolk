...
class BookingRepository extends BaseRepository
{
     ...
     * @return array
     */
    public function getUsersJobsHistory($user_id, array $request)
    {
        $page = $request['page'];
    ...
    public function store($user, array $data)
    {
        ...
        if ($user->user_type == env('CUSTOMER_ROLE_ID')) {
            ...
            if ($data['immediate'] == 'no') {
                $responseMessages = [
                    ['field' => 'due_date', 'message' => 'Du måste fylla in alla fält'],
                    ['field' => 'due_time', 'message' => 'Du måste fylla in alla fält'],
                    ['field' => 'duration', 'message' => 'Du måste fylla in alla fält'],
                ];

                foreach ($responseMessages as $responseMessage) {
                    if (isset($data[$responseMessage['field']]) && $data[$responseMessage['field']] == '') {
                        $response['status'] = 'fail';
                        $response['message'] = $responseMessage['message'];
                        $response['field_name'] = $responseMessage['field'];
                    }
                }
...
//        $jobs = TeHelper::convertJobIdsInObjs($job_ids);
    ...
    private function changeTimedoutStatus($job, $data, $changedTranslator)
    {
        ...
        return [
            'status' => false,
            'message' => 'Failed to change timed out status'
        ];
    }
    ...
    private function changeCompletedStatus($job, $data)
    {
        ...
        if ($data['status'] == 'timedout') {
            if (isset($data['admin_comments']) && $data['admin_comments'] == '') {
                return [
                    'status' => false,
                    'message' => 'Admin comments required'
                ];
            }
    ...
    private function changeStartedStatus($job, $data)
    {
        ...
        if ($data['status'] == 'completed') {
            ...
            if (isset($data['sesion_time']) && $data['sesion_time'] == '') {
                return [
                    'status' => false,
                    'message' => 'Session time is empty'
                ];
            }
        ...
        return [
            'status' => false,
            'message' => 'Failed to change started status'
        ];
    }
    ...
    private function changePendingStatus($job, $data, $changedTranslator)
    {
        ...
        if ($data['admin_comments'] == '' && $data['status'] == 'timedout') {
            return [
                'status' => false,
                'message' => 'Admin comments is empty and status is timed out'
            ];
        }
        ...
        return [
            'status' => false,
            'message' => 'Failed to change pending status'
        ];
    }
    ...
    private function changeWithdrawafter24Status($job, $data)
    {
        if (in_array($data['status'], ['timedout'])) {
            ...
            if (isset($data['admin_comments']) && $data['admin_comments'] == '') {
                return [
                    'status' => false,
                    'message' => 'Admin comments is empty'
                ];
            }
        ...
        return [
            'status' => false,
            'message' => 'Failed to chage withdraw after 24 status'
        ];
    }
    ...
    private function changeAssignedStatus($job, $data)
    {
        ...
        return [
            'status' => false,
            'message' => 'Failed to change assigned status'
        ];
    }
    ...
    private function changeTranslator($current_translator, $data, $job)
    {
        ...
        if (!is_null($current_translator) || (isset($data['translator']) && $data['translator'] != 0) || $data['translator_email'] != '') {
            ...
            } else if (is_null($current_translator) && isset($data['translator']) && ($data['translator'] != 0 || $data['translator_email'] != '')) {
                if ($data['translator_email'] != '') {
                    $data['translator'] = User::where('email', $data['translator_email'])->first()->id;
                }
    ...
    public function acceptJob($job_id, $user)
    {
        ...
        $translatorBooked = Job::isTranslatorAlreadyBooked($job_id, $user->id, $job->due);
        if (!$translatorBooked) {
            $insertTranslator = Job::insertTranslatorJobRel($user->id, $job_id);
            if ($job->status == 'pending' && $insertTranslator) {
                $job->status = 'assigned';
                $job->save();
    ...
    public function acceptJobWithId($job_id, $cuser)
    {
        ...
        $translatorBooked = Job::isTranslatorAlreadyBooked($job_id, $cuser->id, $job->due);
        if (!$translatorBooked) {
            $insertTranslator = Job::insertTranslatorJobRel($cuser->id, $job_id);
            if ($job->status == 'pending' && $insertTranslator) {
                $job->status = 'assigned';
                $job->save();
        ...
        */
        ...
        if ($user->is('customer')) {
            ...
            if ($translator) {
                ...
                $this->notifForCustomerAndTranslator($translator, $job_id, $msg_text);
            }
        } else {
            if ($job->due->diffInHours(Carbon::now()) > 24) {
                ...
                if ($customer) {
                    ...
                    $this->notifForCustomerAndTranslator($customer, $job_id, $msg_text);
                }
    ...
    public function getAll(array $requestdata, $limit = null)
    {
        $cuser = auth()->user();
        ...
        if ($cuser && $cuser->user_type == env('SUPERADMIN_ROLE_ID')) {
            $this->superAdminFilters($allJobs, $requestdata, $to);
        } else {
            $this->userFilters($allJobs, $requestdata, $consumer_type, $to);
        }
    ...
    /**
     * search filters for super admin
     * 
     * @param Model $allJobs
     * @param array $requestdata
     * @param string $to
     */
    private function superAdminFilters($allJobs, $requestdata, $to)
    {
        ...
        if (isset($requestdata['customer_email']) && count($requestdata['customer_email']) && $requestdata['customer_email'] != '') {
            $this->fetchJobsByUserEmails($allJobs, $requestdata['customer_email']);
        }

        if (isset($requestdata['translator_email']) && count($requestdata['translator_email'])) {
            $this->fetchJobsByUserEmails($allJobs, $requestdata['translator_email'], true);
        }
        
        if (isset($requestdata['filter_timetype'])) {
            if ($requestdata['filter_timetype'] == "created") {
                $this->filterJobsByTimeType($allJobs, $requestdata, 'created_at', $to);
            } else {
                $this->filterJobsByTimeType($allJobs, $requestdata, 'due', $to);
            }
        }
     ...
     * @param string $to
     */
    private function userFilters($allJobs, $requestdata, $consumer_type, $to)
    {
        if (isset($requestdata['id']) && $requestdata['id'] != '') {
            $allJobs->where('id', $requestdata['id']);
            $requestdata = array_only($requestdata, ['id']);
    ...
    /**
     * fetch jobs bu user emails
     * 
     * @param Model $allJobs
     * @param string $userEmails
     * @param boolean $jobIds
     */
    private function fetchJobsByUserEmails($allJobs, $userEmails, $jobIds = false) 
    {
        ...
        if ($users->isNotEmpty()) {
            ...
            if ($jobIds) {
                $jobIDs = Translator::whereNull('cancel_at')
     ...
     * @param string $to
     */
    private function filterJobsByTimeType($allJobs, $requestdata, $timeType, $to)
    {
        if (isset($requestdata['from']) && $requestdata['from'] != "") {
    ...
    /**
     * Send notification to customer and translator
     * 
     * @param User $userType
     * @param int $job_id
     * @param string $msg_text
     */
    private function notifForCustomerAndTranslator($userType, $job_id, $msg_text)
    {
        $data = [];
        $data['notification_type'] = 'job_cancelled';

        if ($this->isNeedToSendPush($userType->id)) {
            $users_array = [$userType];
            $this->sendPushNotificationToSpecificUsers($users_array, $job_id, $data, $msg_text, $this->isNeedToDelayPush($userType->id));// send Session Cancel Push to Translaotor
        }
    }
}